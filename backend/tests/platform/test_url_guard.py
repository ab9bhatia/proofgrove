...
async def test_validate_outbound_url_accepts_a_public_endpoint():
    ...
    with patch("proofgrove.evaluation.target.catalog.resolve_endpoint_addresses", dns):
        assert await validate_outbound_url("https://public.example/mcp", "proofgrove") == "https://public.example/mcp"
...
async def test_validate_outbound_url_rejects_a_hostname_that_resolves_to_metadata():
    ...
    with patch("proofgrove.evaluation.target.catalog.resolve_endpoint_addresses", dns):
        with pytest.raises(AgentCatalogError, match="private or local"):
            await validate_outbound_url("https://metadata.internal/mcp", "proofgrove")
...
async def test_validate_outbound_url_rejects_ip_literal_without_dns_lookup():
    with patch("proofgrove.evaluation.target.catalog.resolve_endpoint_addresses") as dns:
        with pytest.raises(AgentCatalogError, match="private and local"):
            await validate_outbound_url("http://127.0.0.1/mcp", "proofgrove")
...
async def test_validate_outbound_url_allows_same_tenant_cluster_service():
    ...
    with patch("proofgrove.evaluation.target.catalog.resolve_endpoint_addresses", dns):
        result = await validate_outbound_url(
            "http://mcp-tool.proofgrove.svc.cluster.local/mcp", "proofgrove"
        )
    assert result == "http://mcp-tool.proofgrove.svc.cluster.local/mcp"
    ...
    with patch("proofgrove.evaluation.target.catalog.resolve_endpoint_addresses") as dns:
        with pytest.raises(AgentCatalogError, match="tenant namespace"):
            await validate_outbound_url(
                "http://mcp-tool.other-tenant.svc.cluster.local/mcp", "proofgrove"
            )
    dns.assert_not_called()
