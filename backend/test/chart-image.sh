...
package="$temp/proofgrove-${version}.tgz"
...
render() {
  helm template image-test "$@" --namespace tenant-image-test \
    ...
    --set-string route.hostname=proofgrove.image-check.example
}
assert_images() {
  local host=$1
  shift
  ...
  for entry in "proofgrove@$api" "eval-ai@$ui" "trace-archive-sink@$sink"; do
  ...
  [[ $(grep -Fc "image: \"${host}/evalai-evalai/evalai/services/proofgrove@${api}\"" <<<"$rendered") -eq 3 ]]
}
...
appInfra:
  ...
  traceArchiveBucket: proofgrove-traces
  postgresHost: example.postgres.database.azure.com
  postgresDatabase: proofgrove
...
for image in proofgrove:latest eval-ai:latest trace-archive-sink:latest; do
  grep -Fq "image: \"$image\"" <<<"$rendered"
done
