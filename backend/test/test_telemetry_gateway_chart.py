...
def render(tenant, profile, gateway_enabled=True):
    ...
    if profile == "azure":
        values["telemetry"]["sink"] = {"ingestAuthTokenSecretName": "eval-hub-ingest-token"}
        values["appInfra"] = {
    ...
    result = subprocess.run(
        ["helm", "template", "eval-hub", str(CHART), "--namespace", f"tenant-{tenant}", "-f", "-"],
...
def test_otlp_credential_stays_tenant_bound(tenant, profile):
    ...
    secret = resources["ExternalSecret", "eval-hub-otlp-api-key"]
    assert secret["metadata"]["namespace"] == f"tenant-{tenant}"
    spec = secret["spec"]
    assert spec["target"]["name"] == "eval-hub-otlp-api-key"
    assert spec["target"]["deletionPolicy"] == "Retain"
    remote_key = f"eval-hub-{tenant}-otlp"
    if profile == "azure":
        assert spec["secretStoreRef"] == {"kind": "SecretStore", "name": "eval-hub-app-infra"}
        store = resources["SecretStore", "eval-hub-app-infra"]["spec"]["provider"]["azurekv"]
        assert store["vaultUrl"] == f"https://{tenant}.vault.azure.net/"
        assert store["serviceAccountRef"] == {"name": "eval-hub"}
        remote_key = "eval-hub-otlp"
    else:
    ...
    auth = resources["SecurityPolicy", "eval-hub-otlp"]["spec"]["apiKeyAuth"]
    assert auth["credentialRefs"] == [{"name": "eval-hub-otlp-api-key"}]
...
def test_disabled_gateway_has_no_otlp_secret_or_policy():
    ...
    assert ("ExternalSecret", "eval-hub-otlp-api-key") not in resources
    assert ("SecurityPolicy", "eval-hub-otlp") not in resources
