...
def render(tenant="alpha", **overrides):
    ...
    args = ["helm", "template", "proofgrove", str(CHART), "--namespace", f"tenant-{tenant}"]
...
def test_managed_postgres_uses_tenant_stores_without_capture(tenant, profile, host):
    ...
    for suffix, store_name, account, client in (
        ("", "proofgrove-app-infra", "proofgrove", "00000000-0000-0000-0000-000000000002"),
        ("-admin", "proofgrove-admin-infra", "proofgrove-admin-secrets", "00000000-0000-0000-0000-000000000003"),
    ):
        secret = resources["ExternalSecret", f"proofgrove-postgres{suffix}-credentials"]["spec"]
        assert secret["secretStoreRef"] == {"name": store_name, "kind": "SecretStore"}
        assert secret["dataFrom"][0]["extract"]["key"] == f"proofgrove-postgres{suffix}"
    ...
    assert {name for kind, name in resources if kind == "Deployment"} == {"proofgrove", f"tenant-{tenant}-eval-ai"}
...
def test_local_postgres_keeps_explicit_vault_mapping():
    ...
    secret = next(d for d in docs if d["kind"] == "ExternalSecret" and d["metadata"]["name"] == "proofgrove-postgres-credentials")
...
def test_role_setup_serializes_privileges_and_removes_truncate():
    result = render()
    assert result.returncode == 0, result.stderr
    job = next(d for d in yaml.safe_load_all(result.stdout) if d and d["kind"] == "Job" and d["metadata"]["name"] == "proofgrove-postgres-role-setup")
    script = job["spec"]["template"]["spec"]["containers"][0]["command"][-1]
    sql = script.split("<<'SQL'\n", 1)[1].rsplit("\nSQL", 1)[0].strip()
    assert sql.startswith("BEGIN;\nSELECT pg_advisory_xact_lock(hashtext('proofgrove-schema-migration'));")
...
def test_default_local_postgres_matches_tenant_bootstrap(tenant):
    result = subprocess.run([
        "helm", "template", "proofgrove", str(CHART), "--namespace", f"tenant-{tenant}",
        "-f", str(CHART / "values-local.yaml"),
        "--set", f"route.hostname=proofgrove.{tenant}.example.invalid",
    ], capture_output=True, text=True, check=True)
    docs = [d for d in yaml.safe_load_all(result.stdout) if d]
    secret = next(d for d in docs if d["kind"] == "ExternalSecret" and d["metadata"]["name"] == "proofgrove-postgres-credentials")
    assert secret["spec"]["dataFrom"][0]["extract"]["key"] == f"proofgrove-{tenant}-postgres"
    assert secret["spec"]["secretStoreRef"] == {"name": "evalai-registry", "kind": "ClusterSecretStore"}
...
def test_deploy_requires_platform_values_before_contacting_cluster(tmp_path, monkeypatch):
    ...
    result = subprocess.run([
        "make", "-f", str(CHART.parent / "Makefile"), "deploy-proofgrove",
        "TENANT=alpha", "ARGOCD_ENV=local",
    ], cwd=tmp_path, capture_output=True, text=True)
    assert result.returncode != 0
    assert "gitops/local/app-values/proofgrove.yaml is not available" in result.stdout
...
def test_default_local_postgres_requires_tenant_namespace(namespace):
    result = subprocess.run([
        "helm", "template", "proofgrove", str(CHART), "--namespace", namespace,
        "--set", "route.hostname=proofgrove.example.invalid",
