"""The product chart owns the UI with the Proofgrove hostname and shared Proofgrove authorization identity."""
...
class CombinedChartTest(unittest.TestCase):
    def test_existing_grant_identity_override(self):
        result = subprocess.run([
            "helm", "template", "tenant-alpha-proofgrove", str(CHART),
            "--namespace", "tenant-alpha", "--set", "route.enabled=true",
            "--set", "route.hostname=proofgrove.alpha.example.invalid",
        ...
        for doc in docs:
            ...
            if doc["kind"] == "Deployment" and doc["metadata"]["name"] == "proofgrove":
        ...
        for tenant in ("alpha", "beta"):
            for public, rum, endpoint in (
                ...
                with self.subTest(tenant=tenant, public=public, rum=rum, endpoint=endpoint):
                    ...
                    args = [
                        "helm", "template", f"{namespace}-proofgrove", str(CHART),
                        "--namespace", namespace,
                    ...
                    self.assertEqual(env["PROOFGROVE_API_URL"]["value"], "http://proofgrove:8000")
                    ...
                    self.assertEqual(route["metadata"]["labels"]["evalai.ai/application-id"], "proofgrove")
                    ...
                    targets = resources["BackendTrafficPolicy", "proofgrove-rate-limit"]["spec"]["targetRefs"]
                    self.assertIn(name + "-httproute", [t["name"] for t in targets])
                    ingress = resources["NetworkPolicy", "allow-proofgrove-ui-to-proofgrove"]["spec"]["ingress"][0]
