"""Tenant identity and application-permission enforcement for Eval Hub.
...
``evalai``). Eval Hub scopes data by the Kubernetes namespace
...
_ROLE_PERMISSIONS = {
    "eval-hub-approver": PERMISSION_GOVERNANCE_APPROVE,
    "eval-hub-reviewer": PERMISSION_GOVERNANCE_REVIEW,
}
GOVERNANCE_ROLES = frozenset(_ROLE_PERMISSIONS)
DEFAULT_RELEASE_APPROVER_ROLE = "eval-hub-approver"
...
async def check_permission(request: Request, permission: str) -> bool:
    ...
    if not settings.authz_check_token:
        logger.error("Eval Hub AuthZ token is not configured")
    ...
    try:
        ...
        logger.error(
            "Eval Hub authorization check failed: %s",
            type(exc).__name__,
            extra={
                "tenant_id": tenant,
    ...
    logger.info(
        "Eval Hub authorization decision",
        extra={
            "tenant_id": tenant,
    ...
    if allowed:
        granted = getattr(request.state, "eval_hub_permissions", set())
        request.state.eval_hub_permissions = {*granted, permission}
    return allowed
...
_TENANT_SCOPE_STATE_KEY = "eval_hub_tenant_scope_checked"
...
def mark_tenant_scope_checked(request: Request) -> None:
    ...
    """
    request.state.eval_hub_tenant_scope_checked = True


class AuthorizationMiddleware:
    """Fail-closed action authorization for every non-health Eval Hub request."""
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        async def guarded_send(message: dict) -> None:
            ...
            if (
                ...
                suppressed = True
                logger.error(
                    "Eval Hub route returned %s with no tenant scope check",
                    message["status"],
...
def resolve_requested_tenant(request: Request, tenant_id: str | None) -> str:
    ...
    ``x-evalai-tenant`` header (the Eval Hub UI BFF always sends this), then
