...
from evalhub.api.dependencies import (
...
from evalhub.api.v1.datasets import _page_window, _paged_envelope
...
from evalhub.evaluation.dataset_bridge import _QUERY_KEYS
...
from evalhub.evaluation.run_service import resolve_active_metrics
from evalhub.evaluation.sample_data import (
    SAMPLE_EXPERIMENTS,
    SAMPLE_TENANT_ID,
...
from evalhub.platform.prompts import MAX_PROMPT_CHARS, parse_prompt_ref, reject_embedded_credentials
from evalhub.platform.quality_contract_templates import QUALITY_CONTRACT_TEMPLATE_BY_ID
from evalhub.platform.resolver import (
    ContractResolutionError,
    resolve_scoring_configuration,
)
from evalhub.settings import settings
from evalhub.tracing.cost import estimate_tokens_cost_usd
from evalhub.tracing.scoring import full_execution_span_checks
...
async def _submit_durable_run(run_id: str) -> None:
    ...
    try:
        ...
        logger.error("eval-hub: Temporal workflow submission for run %s did not complete; left for reconciliation", run_id, extra={"error_type": type(exc).__name__})
...
async def judge_models() -> dict[str, Any]:
    ...
    text-generation ids.
    """
    from evalhub.api.v1.llms import _list_compass_models
...
class DatasetRunRequest(BaseModel):
    ...
    # told apart by prompt without Eval Hub storing prompts.
...
def _scope_options(
    ...
    return scope_options(
        ...
        tool_completion_available=tool_evidence_attestation_available(settings),
...
async def create_run_from_dataset(
    ...
    logger.info(
        "eval-hub: enqueued run %s dataset=%s agent=%s",
        run_id,
        dataset_name,
        opts.agent,
    )
...
async def cancel_run(
    ...
    from evalhub.runs_worker import cancel_active_run

    cancel_active_run(run_id)
    if settings.evaluation_runtime == "temporal":
        from evalhub.orchestrator.temporal import cancel_dataset_run
...
async def usage_overview(
    ...
    Reports only what Eval Hub itself recorded — evaluation runs, their cases'
