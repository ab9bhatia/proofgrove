...
Split out of :mod:`evalhub.platform.payloads` so :mod:`evalhub.logging_config`
...
log fields. ``evalhub.platform.payloads`` re-uses these same patterns for its
...
def redact_for_persistence(value: Any, *, force: bool = False) -> Any:
    ...
    Eval Hub must retain enough evidence for review without persisting bearer
...
def safe_error_message(exc: BaseException) -> str:
    ...
    ``evalhub.errors``). Everything else -- a database driver error carrying
