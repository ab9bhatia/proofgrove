...
from evalhub.configs.app_config import apply_telemetry_profile
...
class Settings(BaseSettings):
    ...
    temporal_task_queue: str = "eval-hub"
    ...
    # Tenant namespace this eval-hub runs in (downward API POD_NAMESPACE). Scopes
    ...
    # ── Dataset generation ─────────────────────────────────────────────────
    # In-process generation tasks (evalhub.datasets.generation_service) run
    ...
    # Archived OTLP trace evidence. Eval Hub has read-only object-store access;
    ...
    trace_archive_bucket: str = "eval-hub-traces"
    ...
    def _resolve_telemetry_profile(self) -> "Settings":
        ...
        if self.app_env.strip().lower() in {"prod", "production"}:
            if (
                ...
                or self.database_url == "postgresql+asyncpg://eval-hub:eval-hub@localhost:5432/eval-hub"
            ):
                raise ValueError(
                    "production requires DATABASE_URL from the eval-hub-postgres-credentials Secret"
                )
            ...
            if not self.authz_check_token.get_secret_value().strip():
                raise ValueError(
                    "production requires AUTHZ_CHECK_TOKEN from the eval-hub authz Secret"
                )
