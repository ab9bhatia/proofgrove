...
from evalhub.api.dependencies import get_evaluation_engine, get_registry_service
from evalhub.db.session import async_engine, async_session
from evalhub.db.store import EvaluationStore, RunCancelledError
from evalhub.evaluation.enums import EvaluationScope, RunStatus, TriggerReason
...
from evalhub.generation.pipeline import generate_and_register
from evalhub.redaction import safe_error_message
from evalhub.settings import settings
from evalhub.tracing.scoring import execute_span_scoring
...
async def _watch_durable_cancellation(
    ...
    while not owner.done():
        ...
        try:
            ...
            logger.warning('eval-hub: could not check cancellation state for run %s', run_id, extra={'error_type': type(exc).__name__})
            continue
        ...
        if job.status == RunStatus.CANCELLED.value:
            logger.info("eval-hub: observed durable stop request for active run %s", run_id)
            owner.cancel()
            return
...
def _start_cancellation_watch(run_id: str) -> asyncio.Task | None:
    ...
    return asyncio.create_task(
        ...
        name=f"evalhub-cancel-watch-{run_id}",
    )
    ...
    try:
        if kind == "generate":
            ...
            if outcome == DATASET_RUN_DEFERRED:
                logger.info("eval-hub: %s job %s awaiting completed traces", kind, run_id)
                return True
        async with async_session() as session:
            await EvaluationStore(session).complete_run_job(run_id)
        logger.info("eval-hub: %s job %s completed", kind, run_id)
    except ReadinessBlockedError as exc:
        logger.warning('eval-hub: %s job %s blocked by readiness: %s', kind, run_id, type(exc).__name__, extra={'error_type': type(exc).__name__})
        async with async_session() as session:
        ...
        logger.info("eval-hub: %s job %s stopped before results were persisted", kind, run_id)
    except Exception as exc:  # noqa: BLE001 — platform failures mark the job FAILED
        logger.error('eval-hub: %s job %s failed', kind, run_id, extra={'error_type': type(exc).__name__})
        async with async_session() as session:
...
async def process_one_deferred_telemetry_job() -> bool:
    ...
    try:
        ...
        async with async_session() as session:
            await EvaluationStore(session).complete_run_job(run_id)
        logger.info("eval-hub: deferred telemetry score completed for %s", run_id)
    except ReadinessBlockedError as exc:
        logger.warning('eval-hub: deferred score %s blocked by readiness: %s', run_id, type(exc).__name__, extra={'error_type': type(exc).__name__})
        async with async_session() as session:
            await EvaluationStore(session).block_run_job(
                run_id,
                str(exc),
        ...
        logger.info("eval-hub: deferred score %s stopped before results were persisted", run_id)
    except Exception as exc:  # noqa: BLE001 — platform failures mark the job FAILED
        logger.error('eval-hub: deferred score %s failed', run_id, extra={'error_type': type(exc).__name__})
        async with async_session() as session:
            await EvaluationStore(session).fail_run_job(run_id, _job_error_message(exc))
    return True
...
async def process_one_completed_telemetry_watch() -> bool:
    ...
    async with async_session() as session:
        ...
        try:
            ...
            logger.error('eval-hub: late telemetry recheck failed for %s', run_id, extra={'error_type': type(exc).__name__})
...
async def process_run_job(run_id: str) -> None:
    ...
    try:
        ...
        logger.info("eval-hub: Temporal job %s stopped before results were persisted", run_id)
...
def application_error_for(exc: BaseException, run_id: str) -> ApplicationError:
    ...
    """
    logger.error("eval-hub: Temporal job %s attempt failed", run_id, extra={"error_type": type(exc).__name__})
...
async def _run_owned_worker(stop: asyncio.Event, poll_interval_seconds: float) -> None:
    ...
    async with async_session() as session:
        ...
        logger.info("eval-hub: worker acquired ownership; recovered %d interrupted jobs", reclaimed)
    logger.info("eval-hub: run worker started")
    while not stop.is_set():
        try:
            iteration = asyncio.create_task(
                process_one_job(register_active=True),
                name="evalhub-run-worker-iteration",
            )
            ...
            logger.error('eval-hub: run worker iteration failed', extra={'error_type': type(exc).__name__})
            processed = False
        if not processed:
            try:
    ...
    logger.info("eval-hub: run worker stopped")
...
async def run_worker_loop(stop: asyncio.Event, poll_interval_seconds: float = 3.0) -> None:
    ...
    while not stop.is_set():
        try:
            ...
            logger.error('eval-hub: compatibility worker ownership lost or unavailable', extra={'error_type': type(exc).__name__})
        if not stop.is_set():
            try:
                await asyncio.wait_for(stop.wait(), timeout=poll_interval_seconds)
            except TimeoutError:
