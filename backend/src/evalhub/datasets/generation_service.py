...
Follows the ``evalhub.runs_worker`` pattern — background asyncio work inside
the API process, no new infrastructure — but keyed by the durable job row in
``dataset_generation_jobs`` (see ``evalhub.datasets.generation_jobs``):
...
from evalhub.generation.pipeline import (  # noqa: PLC2701 — module-internal seams reused deliberately
