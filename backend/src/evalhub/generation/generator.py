...
write a faithful Q&A row, and emit it in Eval Hub's ``agent`` golden CSV schema
...
from evalhub.generation.prompts import build_generation_messages
from evalhub.generation.sources import GroundingSource

logger = logging.getLogger(__name__)

# CSV header for the agent golden schema (matches services/eval-hub/scripts/sample_datasets).
CSV_FIELDS = [
    "input_query",
    "input_tools",
...
class DatasetGenerator:
    ...
    async def generate_row(self, seed: str) -> GeneratedRow | None:
        ...
        if not expected or expected == "INSUFFICIENT_MATERIAL":
            ...
            logger.warning("evalhub.generation: insufficient material for a seed; skipping")
            return None
    ...
    async def generate(self, seeds: list[str]) -> list[GeneratedRow]:
        ...
        for seed in seeds:
            try:
                ...
                logger.warning("evalhub.generation: seed skipped", extra={"error_type": type(exc).__name__})
                continue
            if row is not None:
                rows.append(row)
        return rows
