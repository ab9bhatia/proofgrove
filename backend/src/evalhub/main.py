...
from evalhub.api.health import router as health_router
from evalhub.api.v1.agents import router as agents_router
from evalhub.api.v1.datasets import router as datasets_router
from evalhub.api.v1.evaluation import router as evaluation_router
from evalhub.api.v1.llms import router as llms_router
from evalhub.api.v1.model_providers import router as model_providers_router
from evalhub.api.v1.platform import router as platform_router
from evalhub.api.v1.tracing import router as tracing_router
from evalhub.db.session import async_session, init_db
...
from evalhub.logging_config import configure_logging
from evalhub.platform.authz import AuthorizationMiddleware
from evalhub.platform.rate_limit import RateLimitMiddleware
from evalhub.runs_worker import run_worker_loop
from evalhub.settings import settings
from evalhub.tracing.index_worker import trace_index_worker_loop
...
def _log_worker_exit(task: asyncio.Task) -> None:
    ...
    exc = task.exception()
    if exc is not None:
        logger.critical("eval-hub: background worker %s exited", task.get_name(), extra={"error_type": type(exc).__name__})
...
async def _drain_workers(workers: dict[str, asyncio.Task], grace_seconds: float) -> None:
    ...
    for task in unfinished:
        logger.warning("eval-hub: background worker %s did not stop within %.0fs; cancelling", task.get_name(), grace_seconds)
        task.cancel()
    if unfinished:
        _done, still_running = await asyncio.wait(unfinished, timeout=max(0.0, deadline - loop.time()))
        for task in still_running:
            logger.error("eval-hub: background worker %s is still running at the shutdown deadline; process exit ends it", task.get_name())
...
async def lifespan(app: FastAPI):
    ...
    if setup_invocation_tracing():
        logger.info("eval-hub: evaluation root-span export enabled")

    stop = asyncio.Event()
    app.state.background_workers = {}
    async with AsyncExitStack() as stack:
        if settings.evaluation_runtime == "temporal":
            from evalhub.orchestrator.temporal import reconcile_pending_run_jobs_loop, temporal_worker

            client = await stack.enter_async_context(temporal_worker())
            logger.info("eval-hub: Temporal evaluation worker started")
            ...
            logger.info("eval-hub: local compatibility worker started")
        if settings.trace_index_enabled:
            _start_worker(app, "trace-index-worker", trace_index_worker_loop(stop))
            logger.info("eval-hub: trace index worker started")
        try:
            yield
        finally:
            stop.set()
