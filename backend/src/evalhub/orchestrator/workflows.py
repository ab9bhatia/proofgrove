"""Temporal workflow and idempotent activity for a persisted Eval Hub run job."""
...
# work during that import. ``evalhub.settings`` builds its Settings object at
# import (pydantic-settings expands ``~``), which the sandbox rejects, so a real
# worker failed to validate ``evalhub.dataset-run`` at startup. Both modules
...
async def _execute_dataset_run_job(run_id: str) -> None:
    ...
    """
    from evalhub.runs_worker import application_error_for, process_run_job
...
@activity.defn(name="evalhub.execute-run-job")
...
@activity.defn(name="evalhub.fail-dataset-run-job")
async def fail_dataset_run_job(failure: DatasetRunFailure) -> None:
    ...
    from evalhub.runs_worker import application_error_for, fail_run_job
...
@workflow.defn(name="evalhub.dataset-run")
