...
Maps the eval-hub RAG metrics onto RAGAS single-turn metrics and routes every
...
# eval-hub metric_id -> RAGAS metric class name (in ragas.metrics)
...
def _build_ragas_llm(settings: Settings):
    ...
    if settings.judge_provider == "azure":
        ...
        from evalhub.evaluation.llm_judge import gateway_model_headers
