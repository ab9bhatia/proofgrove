...
Eval Hub used to mint a random parent span id in the outbound ``traceparent``
...
TRACER_NAME = "evalhub.evaluation"
...
def setup_invocation_tracing(*, otlp_endpoint: str | None = None) -> bool:
    ...
    and can join the collector's eval-hub tail-sampled trace.
    """
    ...
    with _provider_lock:
        ...
        if endpoint and not _has_span_processor(provider):
            try:
                ...
                logger.warning("eval-hub: OTLP span exporter is not installed; root spans stay local")
            else:
                ...
                logger.info("eval-hub: evaluation root spans export to %s", traces_endpoint)
        try:
            ...
            logger.exception("eval-hub: OpenAI OpenInference instrumentation could not be enabled")
...
def _eval_resource() -> Resource:
    ...
    attributes: dict[str, str] = {
        "service.name": os.getenv("OTEL_SERVICE_NAME", "eval-hub"),
    }
...
def _stamp_eval_tracestate(span: trace.Span) -> None:
    ...
    eval-hub-collector drops spans whose trace_state lacks this member (unless
    the span name is ``eval_hub.invoke_*``).
    """
