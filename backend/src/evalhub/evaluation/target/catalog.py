...
def normalize_agent_endpoint(endpoint: str, tenant_namespace: str) -> str:
    ...
    Public HTTP(S) endpoints and services in this Eval Hub's own tenant namespace
...
async def resolve_agent_card_request(card_url: str) -> tuple[str, dict[str, str], dict[str, Any]]:
    ...
    for address in addresses:
        if is_blocked_address(address, allow_private=allow_private):
            logger.warning(
                "eval-hub: agent endpoint host %s resolves to blocked address %s", hostname, address
            )
...
async def test_agent_connectivity(
    ...
    *,
    ...
    try:
        ...
        logger.warning(
            "eval-hub: agent-card fetch failed for tenant=%s: %s",
            tenant_namespace,
            type(exc).__name__,
        )
