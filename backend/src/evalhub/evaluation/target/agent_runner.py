...
from evalhub.evaluation.enums import ProvenanceStatus
...
from evalhub.evaluation.target.external import (
    EXTERNAL_PREFIX,
    credential_headers,
    invocation_endpoint,
    resolve_external_target,
)
from evalhub.evaluation.target.sessions import fetch_session_tool_calls
...
async def run_agent_target(
    *,
    ...
    if not external and namespace != settings.pod_namespace:
        ...
        # Eval Hub is deployed one instance per tenant. A "<namespace>/<name>"
    ...
    session_user_id = None if external else f"eval-hub:{namespace}:{uuid4().hex}"
    ...
    if invocation.tool_calls:
        ...
        try:
            ...
            logger.warning(
                "eval-hub: session tool-call fetch failed: %s",
                type(exc).__name__,
            )
