...
from evalhub.tracing.scoring import enqueue_automatic_span_checks
...
async def trace_index_worker_loop(stop: asyncio.Event, interval_seconds: float | None = None) -> None:
    ...
    logger.info("eval-hub: trace index worker started (tenant=%s)", tenant)
    while not stop.is_set():
        ...
        try:
            ...
            logger.error('eval-hub: trace index tick failed', extra={'error_type': type(exc).__name__})
    logger.info("eval-hub: trace index worker stopped")
