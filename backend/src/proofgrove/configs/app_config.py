"""Telemetry profiles for Proofgrove.

Operators select `default` (RabbitMQ + MinIO) or `azure` (Service Bus + Blob)
via `APP_CONFIG_PROFILE` / `TRACE_ARCHIVE_PROFILE`. Proofgrove only reads the
object store; the queue side is owned by proofgrove-collector and trace-archive-sink
