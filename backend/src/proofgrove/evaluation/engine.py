...
from proofgrove.evaluation.evidence_requirements import requires_retrieved_context
from proofgrove.evaluation.judge import Judge, MockJudge, get_judge
...
from proofgrove.evaluation.normalization import normalise_operational, normalise_score, threshold_result, worst_gate
from proofgrove.evaluation.openinference import content_attributes, evaluator_span
from proofgrove.evaluation.rca import diagnose_root_cause, metric_result_failed
...
def _annotator_kind(config, executed_scorer: str | None) -> str:
    """Map Proofgrove adapters onto OpenInference's stable annotator taxonomy."""
...
class EvaluationEngine:
    ...
    def execute(
        ...
        traceability (see ``proofgrove.events`` and ``RunLineage``).
        ...
        for row in rows:
            ...
            for config in configs:
                ...
                try:
                    ...
                    with evaluator_span(
                        tracer=trace.get_tracer("proofgrove.evaluator"),
                        name=f"proofgrove.evaluate.{config.metric_id}",
        ...
        if skipped_oversized:
            logger.info(
                "proofgrove: excluded %s AGENT_OUTPUT_TOO_LARGE row(s) from scoring for run %s",
                skipped_oversized,
                run_id,
            )
            ...
            if root_cause is None:
                from proofgrove.evaluation.models import RootCauseDiagnosis
...
def _metric_not_applicable(metric_id: str, row: EvaluationRow) -> str | None:
    ...
    if metric_id == "rag.document_recall":
        from proofgrove.evaluation.adapters.deterministic_adapter import document_expectation
