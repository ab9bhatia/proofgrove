...
from proofgrove.evaluation.target.a2a_client import externalize_large_tool_results
from proofgrove.evaluation.trace_archive import TraceArchiveReader, tenant_from_namespace
from proofgrove.settings import Settings
from proofgrove.settings import settings as default_settings
...
def _miss_archive(
    ...
    *,
    ...
    if fallback:
        ...
        return
    logger.warning(
        "proofgrove: archived trace %s not available for scoring (%s)",
...
def _defer_archive(
    ...
    *,
    ...
    logger.info(
        "proofgrove: archived trace %s not ready for scoring (%s); waiting for complete, settled evidence",
        trace_id or row.trace_id,
        reason,
    )
    output = dict(row.output_data or {})
...
def _keep_capture_as_fallback(
    ...
    *,
    ...
    logger.warning(
        "proofgrove: archived trace %s not available (%s); preserving A2A response and abstaining from trace-dependent metrics",
        trace_id or row.trace_id,
        reason,
    )
    output = dict(row.output_data or {})
