...
background worker (proofgrove.runs_worker) rather than in the request. This module
...
from proofgrove.evaluation.adapters.dispatcher import build_judge
from proofgrove.evaluation.dataset_bridge import records_to_rows
from proofgrove.evaluation.engine import EvaluationEngine, validate_unique_example_ids
...
from proofgrove.evaluation.target import AgentInvocationError, run_agent_target
from proofgrove.evaluation.target.a2a_client import AgentOutputTooLargeError
from proofgrove.evaluation.target.external import EXTERNAL_PREFIX, resolve_external_target
from proofgrove.evaluation.target.llm_runner import LlmInvocationError, run_llm_target
from proofgrove.evaluation.target.local_workflows import TOOL_EVIDENCE_SOURCE, is_local_agent, model_revision, resolve_local_model
...
from proofgrove.platform.contracts import ResolvedRunManifest, ResolvedScoringConfiguration
    ...
    *,
    ...
    if response_source == "agent":
        ...
        await _run_agent_rows(
            ...
            **({"expected_local_revision": readiness.resolved_provenance.get("revision")} if is_local_agent(agent, settings) else {}),
            selected_tool_ids=selected_tool_ids,
    ...
    if any(row_awaiting_completed_trace(row) for row in rows):
        ...
        logger.info(
            "proofgrove: run %s published response-safe scores with partial evidence; trace-dependent scoring continues in the background",
            run_id,
        )
...
async def _score_and_persist_run(
    *,
    ...
    if run_status == RunStatus.COMPLETED_WITH_PARTIAL_EVIDENCE:
        ...
        logger.warning(
            "proofgrove: run %s completed with partial evidence (%s)",
            run_id,
            diagnostic,
            extra={
                "run_id": run_id,
...
async def _run_agent_rows(
    ...
    *,
    ...
    expected_local_revision: str | None = None,
) -> None:
    ...
    # Freeze the provider/model for the entire run, even if Models is edited
    # while several cases are in flight.
    resolved_local_model = await resolve_local_model(settings) if is_local_agent(agent_ref, settings) else None
    if resolved_local_model is not None and expected_local_revision and model_revision(resolved_local_model) != expected_local_revision:
        raise AgentInvocationError("The local agent's model changed after readiness was checked. Refresh the evaluation and run it again.")
    ...
    async def invoke(row: EvaluationRow) -> None:
        async with semaphore:
            await _run_agent_row(
                ...
                **({"resolved_local_model": resolved_local_model} if resolved_local_model is not None else {}),
            )
...
async def _run_agent_row(
    ...
    *,
    ...
    trace_attributes: dict[str, str] | None = None,
    resolved_external_target=None,
    resolved_local_model: dict | None = None,
) -> None:
    ...
    try:
        ...
        logger.error(
            "proofgrove: agent %s failed for row %s — aborting evaluation run: %s",
            agent_ref,
            row.row_id,
            type(exc).__name__,
        )
    ...
    # A known local executor raises transport/tool failures directly. Its model
    # may legitimately explain a missing record; that answer is evaluation
    # evidence, not a remote-integration outage inferred from its wording.
    failure_reason = None if is_local_agent(agent_ref, settings) else technical_failure_reason(out.response, scoped_tool_calls(out.tool_calls, selected_tool_ids))
    if failure_reason:
        row.latency_ms = round((time.monotonic() - started) * 1000)
        row.trace_unavailable = True
        ...
        logger.error(
            "proofgrove: agent %s row %s integration failure — aborting evaluation run",
            agent_ref,
            row.row_id,
        )
    ...
    if is_local_agent(agent_ref, settings):
        # The in-process executor owns and closes its bounded tool sequence.
        # Preserve that evidence instead of downgrading it to self-reported A2A
        # capture or looking for an archive this local profile does not run.
        if not out.tool_evidence_completion_attested or out.tool_evidence_source != TOOL_EVIDENCE_SOURCE:
            raise AgentInvocationError("Local workflow did not complete its tool evidence capture.")
        row.output_data["response_source"] = "local_workflow_runtime"
        if not uses_a2a_capture_for_scoring(evaluation_scope):
            row.context = [*row.context, *tool_context_texts(out.tool_calls, selected_tool_ids)]
        return
...
def _mark_output_too_large(
    ...
    *,
    ...
    logger.warning(
        "proofgrove: agent %s row %s %s (limit=%s received=%s last_event=%s); continuing experiment without scoring this row",
        agent_ref,
        row.row_id,
...
async def _run_llm_row(
    ...
    *,
    ...
    try:
        ...
        row.invocation_error = str(exc)
        logger.error(
            "proofgrove: LLM %s failed for row %s — aborting evaluation run: %s",
            target_model,
            row.row_id,
            type(exc).__name__,
        )
