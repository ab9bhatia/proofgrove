...
from proofgrove.evaluation.model_providers import target_provider_problem
...
from proofgrove.evaluation.target.discovery import list_tenant_agents
from proofgrove.evaluation.target.external import EXTERNAL_PREFIX, external_summary, resolve_external_target
from proofgrove.evaluation.target.llm_runner import resolve_llm_base_url
from proofgrove.evaluation.target.local_workflows import TOOL_EVIDENCE_SOURCE, WORKFLOWS, get_workflow, local_model_choice, model_revision, workflow_summary
...
from proofgrove.evaluation.trace_hydrator import _reported_usage_total as _reported_token_count
    ...
    for metric in metrics:
        ...
        if family == "agent":
            if not is_agent:
                ...
                if agent_type in {"Declarative", "GuidedWorkflow"} and agent_tools is not None and not agent_tools:
                    applicability = PreRunApplicability.KNOWN_NOT_APPLICABLE
...
async def assess_evidence_readiness(
    *,
    ...
    local = agent.startswith("local:")
    external = agent.startswith(EXTERNAL_PREFIX)
    if local:
        try:
            workflow = get_workflow(agent, settings)
        except AgentInvocationError as exc:
            return EvidenceReadinessResult(status=EvidenceReadiness.BLOCKED,
                details=[ReadinessDetail(code="local_agent_invalid", message=str(exc))], **common)
        model, message = await local_model_choice(settings)
        if model is None:
            return EvidenceReadinessResult(status=EvidenceReadiness.BLOCKED,
                details=[ReadinessDetail(code="local_agent_model_unavailable", message=message)], **common)
        resolved_agent = workflow_summary(workflow, settings, model, message)
        if evaluation_scope == EvaluationScope.FULL_EXECUTION:
            return EvidenceReadinessResult(status=EvidenceReadiness.UNSUPPORTED,
                details=[ReadinessDetail(code="local_full_execution_unsupported",
                    message="Local workflows capture completed tool calls and the final answer. Select tool interactions; full execution requires a complete trace archive.")], **common)
    elif external:
        try:
    ...
    declared_tools = (
        ...
        if resolved_agent.agent_type in {"Declarative", "GuidedWorkflow"}
        else None
    )
    ...
    if requires_tools and not local and not tool_evidence_attestation_available(settings):
        return EvidenceReadinessResult(
            status=EvidenceReadiness.UNSUPPORTED,
            details=[
                ReadinessDetail(
    ...
    return EvidenceReadinessResult(
        ...
        resolved_provenance={
            ...
            "supplied_by": "local workflow registry" if local else ("registered A2A agent card" if external else "kagent-controller"),
            "verification_source": "code-owned local tools and configured model" if local else ("tenant-scoped immutable agent catalog" if external else "tenant-scoped kagent discovery"),
        },
        **common,
    )
...
def classify_evidence_capture(
    ...
    *,
    ...
    if "model_usage" in required:
        ...
        # invocation; provenance remains self-reported because Proofgrove does not
    ...
    if "lifecycle_events" in required:
        # The explicit completion marker on Proofgrove's recorded root span is the
...
def observed_target_provenance(
    *,
    ...
    if response_source == "agent":
        if any(row.output_data.get("response_source") == "local_workflow_runtime" for row in rows):
            # Only the fresh executor's captured identity can attest a local
            # runtime. Do not turn launch-time declarations into observations.
            identities = set()
            for row in rows:
                usage = row.target_usage or {}
                reference, revision = usage.get("local_agent_ref"), usage.get("local_agent_revision")
                model, provider, endpoint = usage.get("model"), usage.get("provider"), usage.get("model_endpoint")
                valid = (
                    row.output_data.get("response_source") == "local_workflow_runtime"
                    and row.tool_evidence_completion_attested
                    and row.tool_evidence_provenance_status == ProvenanceStatus.ATTESTED
                    and row.tool_evidence_source == TOOL_EVIDENCE_SOURCE
                    and reference in {workflow.reference for workflow in WORKFLOWS}
                    and all(isinstance(value, str) and value for value in (revision, model, provider, endpoint))
                    and revision == model_revision({"provider": provider, "model_id": model, "endpoint": endpoint})
                )
                if not valid:
                    identities.clear()
                    break
                identities.add((reference, revision, model, provider))
            observed = next(iter(identities)) if len(identities) == 1 else None
            return {
                "target_type": "agent", "identifier": observed[0] if observed else None,
                "revision": observed[1] if observed else None, "model": observed[2] if observed else None,
                "provider": observed[3] if observed else None,
                "status": ProvenanceStatus.ATTESTED.value if observed else ProvenanceStatus.UNAVAILABLE.value,
                "supplied_by": "local workflow runtime",
                "verification_source": "captured local invocation and selected model configuration" if observed else "local runtime identity incomplete or inconsistent",
            }
        return {
            "target_type": "agent",
