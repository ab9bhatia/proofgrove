...
them in the shared Proofgrove relational database.

The Golden Dataset Registry service is synchronous, so this client uses a
blocking SQLAlchemy engine (see ``proofgrove.db.session.sync_engine``). Each
...
from proofgrove.datasets.models import MAX_ROWS_PER_DATASET, DatasetMetadata
from proofgrove.datasets.versioning import assert_branchable, assert_mutable
...
def _get_dataset_row(
    ...
    *,
    ...
    worker (``proofgrove.runs_worker``) does not yet carry a caller tenant
...
class SqlDatasetStore:
    ...
    def __init__(self) -> None:
        # Mirrors proofgrove.db.session.init_db's gate: production schema is
