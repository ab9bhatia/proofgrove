...
from proofgrove.api.dependencies import get_evaluation_engine, get_evaluation_store
from proofgrove.db.store import EvaluationStore
from proofgrove.evaluation.engine import EvaluationEngine
from proofgrove.evaluation.enums import MetricRequirement, MetricStatus, RunStatus, RunType, TriggerReason
...
from proofgrove.platform.evaluators import EvaluatorDefinition, EvaluatorStatus, MetricPackVersion
from proofgrove.platform.prompts import (
    reject_embedded_credentials,
    validated_prompt_content,
    validated_prompt_id,
)
from proofgrove.platform.quality_contract_templates import (
...
from proofgrove.platform.review import (
...
async def get_capabilities(
    ...
    return {
        "actions": {
            ...
            "approve_governance": _role_granted(request, "proofgrove-approver"),
        }
    }
...
async def save_prompt(
    ...
    _authorize(request, tenant_id, "proofgrove-approver")
    try:
...
async def archive_prompt_version(
    ...
    _authorize(request, resolved_tenant, "proofgrove-approver")
...
async def mark_quality_profile_tested(
    ...
    #
    # An earlier version gated OVERRIDDEN on "proofgrove-approver". That denied
...
async def create_assignment(
    ...
    # sends "proofgrove-ui" — stamped that string into created_by and the column
...
async def open_case_for_review(
    ...
    """
    _authorize(request, await _run_tenant(store, body.run_id), "proofgrove-reviewer")
    try:
...
async def list_review_tasks(
    ...
    if not await store.get_finding(finding_id):
        raise _not_found("Finding", finding_id)
    _authorize(request, await _finding_tenant(store, finding_id), "proofgrove-reviewer")
...
async def list_review_decision_history(
    ...
    _authorize(request, tenant_id, "proofgrove-reviewer")
...
async def replay_regression(
    ...
    _authorize(request, experiment.tenant_id, "proofgrove-reviewer")
