...
from proofgrove.datasets.csv_parser import (
    _METADATA_TAG_KEYS,
    _PRESENTED_TAG_KEYS,
    parse_csv,
)
...
from proofgrove.datasets.postgres_store import expectations_with_expected_tools
...
from proofgrove.evaluation.dataset_bridge import _EXPECTED_KEYS, _QUERY_KEYS
from proofgrove.evaluation.models import RunItemDetail
from proofgrove.evaluation.target.discovery import resolve_catalogued_grounding
from proofgrove.generation.pipeline import resolve_generation_target
...
from proofgrove.platform.url_guard import AgentCatalogError, validate_outbound_url
...
async def generate_dataset(
    ...
    if method == "llms":
        ...
        if not request.model:
            raise HTTPException(status_code=422, detail="a generation model is required")
    ...
    try:
        generation_model, model_endpoint = await resolve_generation_target(request.model, request.model_endpoint)
...
def _job_response(job: dict[str, Any]) -> dict[str, Any]:
    ...
    if payload.get("error"):
        ...
        from proofgrove.generation.public_errors import public_generation_error
        payload["error"] = public_generation_error(str(payload["error"]))
    return payload
