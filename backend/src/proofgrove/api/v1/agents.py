...
from proofgrove.evaluation.target.catalog import AgentCatalogError, test_agent_connectivity
...
from proofgrove.evaluation.target.external import credential_headers, external_summary, invocation_endpoint
from proofgrove.evaluation.target.local_workflows import local_agent_summaries, local_catalog_targets, run_local_workflow
...
from proofgrove.platform.contracts import EvaluationProject, ProjectPurpose, TargetType, TargetVersion
...
def _request_tenant(request: Request) -> str:
    ...
    # A tenant-local Proofgrove may only read or write catalog data for the
...
def _catalog_project_id(tenant_id: str) -> str:
    return str(uuid5(NAMESPACE_URL, f"evalai:proofgrove:agent-catalog:{tenant_id}"))
...
async def _ensure_catalog_project(store: EvaluationStore, tenant_id: str) -> EvaluationProject:
    ...
    project = EvaluationProject(
        ...
        tenant_id=tenant_id,
        name="Agent Catalog",
        description="A2A agents onboarded as Proofgrove evaluation targets.",
        system_type="agent",
        owner="proofgrove",
        purpose=ProjectPurpose.CATALOG_REGISTRY,
        tags={"catalog": "agents", "registry": "evaluation-targets"},
        created_by="proofgrove",
    )
    try:
...
def _platform_target_version_id(tenant_id: str, agent: AgentSummary) -> str:
    return str(
        uuid5(
            NAMESPACE_URL,
            f"evalai:proofgrove:kagent-target:{tenant_id}:{agent.id}:{agent.revision}",
        )
    )
...
async def _sync_platform_agent_catalog(store: EvaluationStore, tenant_id: str) -> list[TargetVersion]:
    ...
    if local_lab_mode(settings) is not None:
        builtins = local_catalog_targets(await local_agent_summaries(settings), tenant_id, _catalog_project_id(tenant_id))
        return [*builtins, *(target for target in existing if target.configuration.get("catalog_source") == "a2a_agent_card")]
    try:
    ...
    if discovered:
        ...
        for agent in discovered:
            ...
            target = TargetVersion(
                ...
                created_by="proofgrove-agent-sync",
            )
            try:
...
async def list_agents(
    ...
    local = local_lab_mode(settings) is not None
    if local:
        agents = await local_agent_summaries(settings)
        if ready_only:
            agents = [agent for agent in agents if agent.ready]
    else:
        try:
            ...
            logger.warning("proofgrove: agent discovery failed", extra={"error_type": type(exc).__name__})
            if not external_targets:
                raise HTTPException(status_code=502, detail="kagent agent listing failed") from exc
            agents = []

    target_keys = [(agent.id, "local") if local else (_platform_target_id(agent), _PLATFORM_TARGET_ENVIRONMENT) for agent in agents]
...
async def list_mcp_servers(request: Request) -> list[ToolServerSummary]:
    ...
    try:
        ...
        logger.warning("proofgrove: tool-server discovery failed", extra={"error_type": type(exc).__name__})
...
async def list_agent_catalog(
    ...
    if local_lab_mode(settings) is not None:
        ...
        builtins = local_catalog_targets(await local_agent_summaries(settings), tenant_id, _catalog_project_id(tenant_id))
        return [target.model_dump(mode="json") for target in [*builtins, *targets]
                if target.configuration.get("catalog_source") in {"local_workflow", "a2a_agent_card"}]
    ...
    try:
        ...
        logger.warning("proofgrove: agent catalog sync failed", extra={"error_type": type(exc).__name__})
...
class LocalAgentInvokeRequest(BaseModel):
    query: str = Field(min_length=1, max_length=16000)


@router.post("/local/{slug}/invoke")
async def invoke_local_agent(slug: str, body: LocalAgentInvokeRequest, request: Request) -> dict:
    """Try the same bounded local runtime used by full evaluation runs."""
    _request_tenant(request)
    if local_lab_mode(settings) is None:
        raise HTTPException(status_code=404, detail="Local workflow agents are unavailable in this profile.")
    try:
        output = await run_local_workflow(settings=settings, target_endpoint="local:" + slug, query=body.query)
    except AgentInvocationError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from None
    return {"response": output.response, "invocation_id": output.invocation_id,
            "trace_id": output.trace_id, "span_id": output.span_id,
            "tool_calls": [call.model_dump(mode="json") for call in output.tool_calls],
            "latency_seconds": output.latency_seconds, "model": (output.target_usage or {}).get("model"),
            "execution_mode": "guided_local_workflow", "fixture_source": "synthetic_local_fixtures",
            "tool_evidence_source": output.tool_evidence_source, "external_side_effects": False}
