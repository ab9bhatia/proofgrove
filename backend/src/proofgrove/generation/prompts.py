...
Rules:
- Return exactly the requested number of records. The Size field takes precedence over any count in the instruction.
...
def build_prompt_batch_messages(
    ...
    *,
    ...
    system = (
        ...
        "golden evaluation cases from a single user instruction. Return ONLY valid JSON. The requested Size overrides any count in the instruction. Keep each question and expected answer concise."
    )
...
def prompt_records_schema(num_rows: int) -> dict:
    """Constrain local model decoding; generated expectations still need review."""
    return {
        "type": "object", "additionalProperties": False, "required": ["records"],
        "properties": {"records": {
            "type": "array", "minItems": num_rows, "maxItems": num_rows,
            "items": {
                "type": "object", "additionalProperties": False,
                "required": ["question", "expected_output", "risk"],
                "properties": {
