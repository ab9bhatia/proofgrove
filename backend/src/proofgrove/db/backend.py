"""Database-product abstraction for Proofgrove persistence.

Proofgrove talks to relational stores through SQLAlchemy. The concrete product
...
def get_backend(name_or_dialect: str) -> DatabaseBackend:
    ...
    raise UnsupportedDatabaseBackendError(
        ...
        "Analytical warehouses (e.g. Databricks SQL) are not OLTP backends for Proofgrove."
    )
...
def ensure_async_url(database_url: str, backend: DatabaseBackend | None = None) -> str:
    ...
    if not backend.transactional_oltp:
        raise UnsupportedDatabaseBackendError(
            f"Backend {backend.name!r} is not a supported transactional store for Proofgrove."
        )
