"""Safe Alembic entrypoint for fresh and legacy Proofgrove databases."""
...
from proofgrove.db import models  # noqa: F401 - register all ORM tables
from proofgrove.db.session import Base, sync_engine
...
def legacy_schema_revision(connection: Connection) -> str | None:
    ...
    Older Proofgrove versions bootstrapped ORM tables with ``create_all`` and did
...
def main() -> None:
    ...
    with engine.begin() as connection:
        ...
        if connection.dialect.name == "postgresql":
            connection.exec_driver_sql("SELECT pg_advisory_xact_lock(hashtext('proofgrove-schema-migration'))")
