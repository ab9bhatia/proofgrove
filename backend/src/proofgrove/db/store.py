...
from proofgrove.evaluation.evidence_requirements import canonicalize_capture_requirements
...
from proofgrove.platform.authz import namespace_for_tenant, tenant_id_candidates, tenants_match, validate_governance_roles
...
from proofgrove.platform.evaluators import (
    EvaluatorDefinition,
    EvaluatorStatus,
    MetricPackVersion,
...
from proofgrove.tracing.models import SPAN_INDEX_REV, TraceLifecycleState
...
class EvaluationStore:
    ...
    async def save_run(
        ...
        *,
        ...
        if self.session.get_bind().dialect.name == "postgresql":
            lock_id = int.from_bytes(hashlib.sha256(f"proofgrove:save_run:{exp_id}".encode()).digest()[:8], signed=True)
    ...
    async def usage_overview(
        ...
        *,
        ...
        Covers evaluation traffic Proofgrove itself recorded: completed runs from
    ...
    async def _usage_aggregate(
        ...
        *,
        ...
        from proofgrove.evaluation.adapters.deterministic_adapter import usage_field_value
    ...
    async def list_agent_targets(self, tenant_id: str) -> list[TargetVersion]:
        """List tenant-scoped agent targets for the Proofgrove Agent Catalog."""
    ...
    async def list_llm_targets(self, tenant_id: str) -> list[TargetVersion]:
        """List tenant-scoped custom LLM targets for the Proofgrove LLM Catalog."""
