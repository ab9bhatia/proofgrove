"""Bounded, Phoenix-style application rate limiting for Proofgrove.
...
approved ClusterIP callers that do not traverse the Proofgrove HTTPRoute.
"""
...
# Kept in sync with proofgrove.platform.authz._PROBE_PATHS -- both guard the
...
def _path_is_or_is_under(path: str, roots: tuple[str, ...]) -> bool:
    ...
    probe allowlist -- see ``proofgrove.platform.authz._path_is_or_is_under``,
    which this mirrors.
    """
...
class ServerRateLimiter:
    ...
    available. Proofgrove additionally caps and expires the cache so attacker-
