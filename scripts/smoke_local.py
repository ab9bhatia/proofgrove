...
datasets = json.loads(get(ui + '/api/proofgrove/datasets'))
...
runs = json.loads(get(ui + '/api/proofgrove/evaluation/runs?tenant_id=tenant-local-classroom'))
...
starter = json.loads(get(ui + '/api/proofgrove/datasets/nova_live_starter_v1/records'))
assert len(starter) == 5 and all(row['tags']['source'] == 'synthetic-live-starter' for row in starter)
refunds = json.loads(get(ui + '/api/proofgrove/datasets/nova_refunds_golden_v1/records'))
assert len(refunds) == 8 and all(row['tags']['source'] == 'synthetic-refund-demo' for row in refunds)
prompts = json.loads(get(ui + '/api/proofgrove/platform/prompts'))
...
if mode['live']:
    ...
    models = json.loads(get(ui + '/api/proofgrove/evaluation/llm-catalog'))
    assert any(m['model_id'] == mode['model'] and m['endpoint'] == mode['endpoint'] for m in models)
    providers = json.loads(get(ui + '/api/proofgrove/evaluation/model-providers'))
