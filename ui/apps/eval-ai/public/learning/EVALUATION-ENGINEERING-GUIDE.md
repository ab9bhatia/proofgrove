...
For Nova, a customer paid AED 250 for defective headphones. The store has received the return and confirmed the defect; its fictional policy permits a refund. The expected refund is **AED 250, once, against the correct order and original payment method**. “Refunded AED 250” is a claim. The request arguments and independently verified refund record are evidence. If a faulty adapter accepts missing currency and writes USD 250, the action is wrong even if the sentence looks perfect. A conforming adapter should reject missing currency before any effect. Evaluation can expose this before release; runtime validation and idempotency protect execution.
