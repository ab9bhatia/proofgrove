...
describe("provider settings API transport", () => {
  it("reads provider status through the BFF", async () => {
    ...
    expect(sessionAwareFetch).toHaveBeenCalledWith("/api/eval-hub/evaluation/model-providers", expect.objectContaining({ cache: "no-store" }));
  });
  it("sends an OpenAI secret only in the explicit POST body", async () => {
    ...
    expect(url).toBe("/api/eval-hub/evaluation/model-providers/openai");
  ...
  it("sets the provider-qualified default through the BFF", async () => {
    ...
    expect(sessionAwareFetch).toHaveBeenCalledWith("/api/eval-hub/evaluation/model-providers/default", expect.objectContaining({ method: "PUT", body: JSON.stringify({ provider: "ollama", model_id: "installed-model" }) }));
  });
  it("disconnects through DELETE and returns updated status", async () => {
    ...
    expect(sessionAwareFetch).toHaveBeenCalledWith("/api/eval-hub/evaluation/model-providers/openai", expect.objectContaining({ method: "DELETE" }));
  });
});
