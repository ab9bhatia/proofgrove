  ...
  return (
    <section aria-labelledby="prepared-nova-title" className="panel mt-8 p-5 sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="eval-hub-eyebrow text-xs text-evalai-purple">Prepared model evaluation</p>
          <h2 id="prepared-nova-title" className="mt-2 text-lg font-semibold">Evaluate a model on Nova refund cases</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">Send eight refund questions to your configured model. Compare its generated answers with reviewed reference answers using the prepared prompt, metrics and project.</p>
        </div>
      ...
      {loading ? <p className="mt-5 text-sm text-muted-foreground" role="status">Checking the saved artifacts…</p> : <>
        ...
        <div className="mt-6 flex flex-wrap items-center gap-3">
          {state.liveReady ? <Link className="inline-flex min-h-11 items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" href={preparedEvaluationHref("llm")}>Start evaluation <ArrowRight className="size-4" aria-hidden="true" /></Link> : <span className="inline-flex min-h-11 items-center rounded-md border px-4 py-2 text-sm text-muted-foreground">Configure a model and complete the artifacts to start evaluation</span>}
