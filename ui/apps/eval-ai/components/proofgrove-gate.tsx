...
import {
  ...
  proofgroveGateCopy,
  type ProofgroveGateState,
} from "@/lib/api-errors";

type State = "checking" | "up" | ProofgroveGateState;

const POLL_MS = 5000;

const STATE_ICON: Record<ProofgroveGateState, LucideIcon> = {
  offline: WifiOff,
 ...
 * to the specific {@link ProofgroveGateState} for its signal. Kept side-effect free
 * so callers only ever `setState` after awaiting (never synchronously).
 */
async function probeProofgrove(): Promise<"up" | ProofgroveGateState> {
  // No point probing when the browser already knows it is offline.
  if (typeof navigator !== "undefined" && navigator.onLine === false) {
    return classifyProofgroveGate({ online: false });
  }
  ...
  try {
    ...
    return classifyProofgroveGate({ networkError: true });
  }

  if (!res.ok) {
    return classifyProofgroveGate({ status: res.status });
  }

  try {
    const body = (await res.json()) as { available?: boolean };
    if (body.available === true) return "up";
    return classifyProofgroveGate({ available: false });
  } catch {
    return classifyProofgroveGate({ status: 502 });
  }
}
 ...
 * HTTP status, or the health flag) via {@link classifyProofgroveGate} rather than
 * collapsing everything into "not provisioned".
 */
export function ProofgroveGate({ children }: { children: React.ReactNode }) {
  ...
  const check = useCallback(() => probeProofgrove().then(setState), []);
  ...
  useEffect(() => {
    ...
    const tick = () => {
      void probeProofgrove().then((next) => {
        if (cancelled) return;
        setState(next);
  ...
  return <ProofgroveGatePanel state={state} onRetry={retry} onSignInAgain={signInAgain} />;
}
 ...
 */
export function ProofgroveGatePanel({
  state,
  onRetry,
  onSignInAgain,
}: {
  state: ProofgroveGateState;
  onRetry: () => void;
  onSignInAgain: () => void;
}) {
  const copy = proofgroveGateCopy(state);
