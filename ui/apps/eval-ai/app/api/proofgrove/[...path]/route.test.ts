...
describe("Proofgrove BFF proxy", () => {
  ...
  it.each(["classroom", "tenant-classroom"])("forwards the exact slug %s from pod context, ignoring browser headers", async (slug) => {
    ...
    const response = await proxy(
      new Request("http://localhost/api/proofgrove/datasets", {
  ...
  it("does not reflect raw backend failures", async () => {
    ...
    const response = await proxy(
      new Request("http://localhost/api/proofgrove/datasets/private-run"),
  ...
  it("preserves retry-after for a bounded rate-limit response", async () => {
    ...
    const response = await proxy(
      new Request("http://localhost/api/proofgrove/evaluation/runs", {
        method: "POST",
        body: "{}",
      }),
  ...
  it("forwards a coded dict detail as the bounded problem contract only", async () => {
    ...
    const response = await proxy(
      new Request("http://localhost/api/proofgrove/evaluation/dataset/run", {
        method: "POST",
        body: "{}",
      }),
  ...
  it("forwards validation-error arrays as bounded per-field details", async () => {
    ...
    const response = await proxy(
      new Request("http://localhost/api/proofgrove/evaluation", {
        method: "POST",
        body: "{}",
      }),
      context("evaluation"),
  ...
  it("does not forward arbitrary string details", async () => {
    ...
    const specific = await proxy(
      new Request("http://localhost/api/proofgrove/evaluation", { method: "POST", body: "{}" }),
      context("evaluation"),
    );
  ...
  it.each(["docs", "redoc", "openapi.json"])(
    ...
    async (segment) => {
      ...
      const response = await proxy(
        new Request(`http://localhost/api/proofgrove/${segment}`),
        context(segment),
      );
  ...
  it.each([
    ...
    const response = await proxy(
      new Request(`http://localhost/api/proofgrove/${segments.map(encodeURIComponent).join("/")}`),
      context(...segments),
    );
  ...
  it("still proxies a known first segment", async () => {
    ...
    const response = await proxy(
      new Request("http://localhost/api/proofgrove/tracing"),
      context("tracing"),
    );
