...
// the sibling static route at app/api/eval-hub/tenant/route.ts, kept here as
...
function logProxyFailure(fields: {
  ...
  status: number;
  durationMs: number;
  event: "eval_hub_proxy_rejected" | "eval_hub_proxy_unavailable";
}) {
...
export async function proxy(req: Request, ctx: Ctx): Promise<Response> {
  ...
  try {
    ...
    logProxyFailure({
      ...
      event: "eval_hub_proxy_unavailable",
    });
    return errorResponse(503, requestId);
  }

  const search = new URL(req.url).search;
  const target = `${evalHubBaseUrl()}/${path.map(encodeURIComponent).join("/")}${search}`;
